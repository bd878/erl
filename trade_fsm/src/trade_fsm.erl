...
%% Mention that you are ready for a trade.
...
%% Tells ther other fsm that the user is currently waiting
%% for the ready state.
am_ready(OtherPid) ->
...
%% This could be messages to ther clients,
...
%% Other side has accepted our offer. Move to negotiate state
...
negotiation({make_offer, Item}, S=#state{ownitems=OwnItems}) ->
...
wait(are_you_ready, S=#state{}) ->
  ...
  notice(S, "asked of ready, and I am. Waiting for same reply", []),
  {next_state, wait, S};
...
commit(S = #state{}) ->
  io:format("Transaction completed for ~s. "
            ...
            "This operation should havesome atomic save "
            "in a database.~n",
            [S#state.name, S#state.ownitems, S#state.otheritems]).

%% The other player has sent this cancel event
%% stop wheatever we're doing and shut down!
...
%% This cancel event comes from the client. We must warn ther other
